    ...
    async function loadData() {
        try {
            ...
            data = csvText.split('\n').slice(1).map(row => {
                ...
                // 각 데이터의 앞뒤 공백을 제거하여 검색 정확도 향상
                return {
                    year: columns[0] ? columns[0].trim() : '',
                    session: columns[2] ? columns[2].trim() : '',
                    period: columns[3] ? columns[3].trim() : '',
                    husband: columns[8] ? columns[8].trim() : '',
                    wife: columns[10] ? columns[10].trim() : '',
                    parish: columns[7] ? columns[7].trim() : ''
                };
            });
        } catch (error) {
